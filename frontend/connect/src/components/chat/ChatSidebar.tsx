import { Avatar, Box, Button, CircularProgress, Modal, Typography, TextField } from "@mui/material";
...
const ChatSidebar = () => {
    ...
    const OnUserSelect = (user: ProfileResponse) => {
        console.log("selected user: ", user);
        
    }


    const startNewChat = (chat: InboxChat) => {
        socket!.emit('start-chat', chat.chatId);
        setOpen(false);
    };
        ...
        <>
            <div className="h-full overflow-hidden flex flex-col gap-2">
                <div className='p-2 flex items-center justify-between'>
                    <Logo />
                    <div onClick={handleOpen} className="p-2 cursor-pointer text-gray-400 hover:text-gray-400 transition-colors duration-300 ease-in-out">
           ...
           <UserSearchModal open={open} onClose={handleClose} onSelectUser={OnUserSelect}/>
        </>
    );
};
